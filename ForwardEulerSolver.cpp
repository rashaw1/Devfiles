//Implements ForwardEulerSolver class
...
//Constructor
ForwardEulerSolver::ForwardEulerSolver(double (*pFunction)(double, double),
...
double ForwardEulerSolver::Function(double y, double t)
{
  return (*mpFunction)(y, t);
}

/*The following implements the Forward Euler iterative one-step method
  and prints the (y, t) values to a specified file stream.
  See e.g. Introduction to Numerical Analysis by E. Suli and D. Mayers  */
void ForwardEulerSolver::SolveEquation(std::ofstream& output)
{
  ...
  yVals[0] = initValue;
  for(int i = 1; i<N+1; i++){
    yVals[i] = yVals[i-1] + stepSize*Function(yVals[i-1], tVals[i-1]);
  }
